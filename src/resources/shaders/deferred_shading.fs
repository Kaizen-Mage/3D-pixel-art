...
vec3 Linearize(vec3 a){
    ...
    float depth = (2.0*zNear)/(zFar + zNear - a.r*(zFar - zNear));
    return vec3(depth);   
}
//Vars
...
void main() {
    ...
    vec3 crease_edge=vec3(0.8,0.0,0.0);
    vec3 silhouette =vec3(0.0,0.2,1.0);
   
    float depthThreshold = 0.02; // tweak this
    if(normal_center.w>0.5){
        useOutline=true;
    }
    if(abs(depth_center - depth_left).r > depthThreshold || abs(depth_center - depth_right).r > depthThreshold ||
